...
function editorScreenshot() {
    var element = $(".editorChatScreen")[0];

    var options = {
        style: {
            transform: 'scale(2)', // Increase the scaling factor for higher resolution
            'transform-origin': 'top left' // Set the origin of scaling
        },
        width: element.offsetWidth * 2, // Adjust the width based on scaling factor
        height: element.offsetHeight * 2, // Adjust the height based on scaling factor
        quality: 1 // Set the quality of the resulting image (0 to 1)
    };
    ...
    domtoimage.toBlob(element, options)
        .then(function(blob) {
            ...
            // Ensure the link is in the body, so 'click' is allowed
...
function editorBonusCode(text) {
    return;
    if ((text === "ᕕ(◠ڼ◠)ᕗ") && (momotalkCurrentPerson === "USER") && (!momotalkLoadBonus)) {
        momotalkLoadBonus = true;
        $.ajax({
            url: "https://wiki.biligame.com/ba/index.php?title=MediaWiki:MomotalkBonus.json&action=raw",
            dataType: "text",
            async: false, // Make the AJAX request synchronous
            success: function(data) {
                let bonus = JSON.parse(data);
                momotalkData = {...momotalkData, ...bonus};
                if (momotalkStudentSelectorList.length === 0) {
                    for (let student in momotalkData) {
                        if (student !== "USER") {
                            momotalkStudentSelectorList.push(student);
                ...
                for (let item in bonus) {
                    momotalkStudentSelectorList.push(item);
                }
            },
            error: function() {
              console.log("Failed to obtain bonus data.");
            }
          });
    }
}
